export default {
    "brushAndZoomAreaSettings": {},
    "chartSettings": {
        "lollipopInnerPadding": 30,
        "isHasSubCategories": false,
        "orientation": "horizontal",
        "isAutoLollipopWidth": true,
        "isLollipopDistanceChange": false,
        "isShowZeroBaseLine": true,
        "zeroBaseLineColor": "rgba(211, 211, 211, 1)",
        "zeroBaseLineSize": 1,
        "showConnectingLine": false,
        "connectingLineColor": "rgba(0, 0, 0, 1)",
        "connectingLineWidth": 1,
        "connectingLineStyle": "dotted",
        "isIBCSEnabled": false,
        "isResetInIBCSPressed": false
    },
    "cutAndClipAxisSettings": {},
    "dataColorsSettings": {
        "fillMin": "rgba(13, 178, 250, 1)",
        "fillMid": "rgba(238, 14, 188, 1)",
        "fillMax": "rgba(16, 236, 159, 1)",
        "isAddMidColor": false,
        "fillType": "byCategory",
        "isFillTypeChanged": true,
        "singleColor1": "rgba(0, 99, 178, 1)",
        "singleColor2": "rgba(156, 195, 213, 1)",
        "gradientColors": [
            "rgba(186, 72, 239, 1)",
            "rgba(127, 88, 246, 1)",
            "rgba(75, 132, 244, 1)"
        ],
        "categoryColors": [
            {
                "name": "Wednesday",
                "marker": "rgba(245, 166, 35, 1)"
            },
            {
                "name": "Tuesday",
                "marker": "rgba(31, 132, 255, 1)"
            },
            {
                "name": "Thursday",
                "marker": "rgba(246, 48, 141, 1)"
            },
            {
                "name": "Sunday",
                "marker": "rgba(15, 242, 198, 1)"
            },
            {
                "name": "Saturday",
                "marker": "rgba(244, 220, 75, 1)"
            },
            {
                "name": "Monday",
                "marker": "rgba(178, 149, 250, 1)"
            },
            {
                "name": "Friday",
                "marker": "rgba(158, 250, 170, 1)"
            }
        ],
        "numberOfClasses": 8,
        "schemeColors": [
            "rgb(204,233,255)",
            "rgb(179,222,255)",
            "rgb(153,211,255)",
            "rgb(128,200,255)",
            "rgb(102,189,255)",
            "rgb(73,178,255)",
            "rgb(43,167,255)",
            "rgb(22,142,230)"
        ],
        "colorBlindSafe": false,
        "colorScheme": "Blue",
        "reverse": false,
        "isGradient": true,
        "negativeColor": "rgba(229, 51, 39, 1)",
        "positiveColor": "rgba(141, 200, 61, 1)",
        "gradientAppliedToMeasure": "marker1Color",
        "isCustomizeOthersColor": true,
        "othersColor": "rgba(84, 84, 84, 1)"
    },
    "dataLabelsSettings": {
        "show": false,
        "isShowBestFitLabels": true,
        "measure1": {
            "color": "rgba(93, 93, 93, 1)",
            "borderColor": "rgba(255, 255, 255,1)",
            "borderWidth": 1,
            "orientation": "horizontal",
            "placement": "outside",
            "fontSize": 12,
            "fontFamily": "\"Segoe UI\", wf_segoe-ui_normal, helvetica, arial, sans-serif",
            "showBackground": false,
            "backgroundColor": "rgba(93, 93, 93, 1)",
            "fontStyle": [],
            "isAutoFontSize": true,
            "showLabelsBelowReferenceLine": false,
            "isColorChanged": false,
            "textColorTypes": "contrast",
            "applyFor": "only patterns",
            "isShowBGChangedWhenPatternApplied": false,
            "isTextColorTypeChanged": false,
            "isShowBackgroundChange": false,
            "customLabel": "",
            "displayType": "first-last-min-max"
        },
        "measure2": {
            "color": "rgba(93, 93, 93, 1)",
            "borderColor": "rgba(255, 255, 255,1)",
            "borderWidth": 1,
            "orientation": "horizontal",
            "placement": "outside",
            "fontSize": 12,
            "fontFamily": "\"Segoe UI\", wf_segoe-ui_normal, helvetica, arial, sans-serif",
            "showBackground": false,
            "backgroundColor": "rgba(93, 93, 93, 1)",
            "fontStyle": [],
            "isAutoFontSize": true,
            "showLabelsBelowReferenceLine": false,
            "isColorChanged": false,
            "textColorTypes": "contrast",
            "applyFor": "only patterns",
            "isShowBGChangedWhenPatternApplied": false,
            "isTextColorTypeChanged": false,
            "isShowBackgroundChange": false,
            "customLabel": "",
            "displayType": "first-last-min-max"
        }
    },
    "dynamicDeviationSettings": {},
    "errorBarsSettings": {},
    "gridLinesSettings": {},
    "lineSettings": {
        "show": true,
        "lineType": "dotted",
        "lineWidth": 3,
        "lineColor": "rgba(128, 128, 128, 1)",
        "isApplyMarkerColor": true
    },
    "markerSettings": {
        "markerType": "shape",
        "markerStyleType": "marker1Style",
        "marker1Style": {
            "isAutoMarkerSize": false,
            "markerSize": 25,
            "markerShape": "default",
            "markerChart": "donut",
            "markerShapePath": "",
            "markerShapeBase64Url": "",
            "dropdownMarkerType": "circle",
            "isShowMarkerOutline": true,
            "outlineWidth": 2,
            "outlineColor": "rgba(46, 46, 46, 1)",
            "sameOutlineAsMarkerColor": false,
            "showOutlineOnly": false
        },
        "marker2Style": {
            "isAutoMarkerSize": false,
            "markerSize": 20,
            "markerShape": "default",
            "markerChart": "donut",
            "markerShapePath": "",
            "markerShapeBase64Url": "",
            "dropdownMarkerType": "circle",
            "isShowMarkerOutline": true,
            "outlineWidth": 1,
            "outlineColor": "rgba(128, 128, 128, 1)",
            "sameOutlineAsMarkerColor": true,
            "showOutlineOnly": false
        },
        "isAutoLollipopTypePie": false,
        "isAutoLollipopTypeImage": false
    },
    "patternSettings": {},
    "raceChartSettings": {
        "isEnabled": true,
        "allowTransition": true,
        "transitionDuration": 500,
        "dataChangeInterval": 500,
        "labelColor": "rgba(15, 61, 110, 1)",
        "labelFontSize": 30,
        "isLabelAutoFontSize": false,
        "labelFontFamily": "\"Calibri\"",
        "tickerButtonRadius": 30,
        "isTickerButtonAutoRadius": true,
        "tickerButtonColor": "rgba(102,102,102,1)",
        "fontStyles": [
            "bold"
        ],
        "isShowLabelBackground": false,
        "backgroundColor": "rgba(0,0,0,1)",
        "placement": "bottom"
    },
    "rankingSettings": {},
    "referenceLinesSettings": {},
    "showBucket": {},
    "smallMultiplesSettings": {},
    "sorting": {},
    "xAxisSettings": {
        "show": true,
        "position": "bottom",
        "isLogarithmScale": false,
        "isDisplayTitle": false,
        "titleName": "Month",
        "titleColor": "rgba(84, 84, 84, 1)",
        "titleFontSize": 13,
        "titleFontFamily": "\"Segoe UI\", wf_segoe-ui_normal, helvetica, arial, sans-serif",
        "titleStyling": [
            "bold"
        ],
        "isDisplayLabel": true,
        "labelColor": "rgba(84, 84, 84, 1)",
        "labelFontFamily": "\"Segoe UI\", wf_segoe-ui_normal, helvetica, arial, sans-serif",
        "labelFontSize": 12,
        "isLabelAutoTilt": true,
        "labelTilt": 0,
        "labelCharLimit": 20,
        "labelStyling": [],
        "isShowAxisLine": false,
        "axisLineColor": "rgba(84, 84, 84, 1)",
        "isLabelAutoCharLimit": true,
        "categoryType": "categorical",
        "isMaximumRangeEnabled": false,
        "maximumRange": 0,
        "isMinimumRangeEnabled": false,
        "minimumRange": 0,
        "isInvertRange": false,
        "isAutoDateFormat": true,
        "dateFormat": "DD:MM:YYYY hh:mm A",
        "customDateFormat": "DD:MM:YYYY hh:mm A",
        "numberFormatting": {
            "show": true,
            "valueType": "absolute",
            "decimalSeparator": ".",
            "thousandsSeparator": ",",
            "decimalPlaces": 1,
            "scaling": "auto",
            "prefix": "",
            "suffix": "",
            "scalingLabel": false,
            "thousandScalingLabel": "K",
            "millionScalingLabel": "M",
            "billionScalingLabel": "B",
            "trillionScalingLabel": "T"
        }
    },
    "yAxisSettings": {
        "show": true,
        "position": "left",
        "isLogarithmScale": false,
        "isDisplayTitle": false,
        "titleName": "Website Visits",
        "titleColor": "rgba(84, 84, 84, 1)",
        "titleFontSize": 13,
        "titleFontFamily": "\"Segoe UI\", wf_segoe-ui_normal, helvetica, arial, sans-serif",
        "titleStyling": [
            "bold"
        ],
        "isDisplayLabel": true,
        "labelColor": "rgba(84, 84, 84, 1)",
        "labelFontFamily": "\"Segoe UI\", wf_segoe-ui_normal, helvetica, arial, sans-serif",
        "labelFontSize": 12,
        "labelCharLimit": 20,
        "labelStyling": [],
        "isShowLabelsAboveLine": false,
        "isShowAxisLine": false,
        "axisLineColor": "rgba(84, 84, 84, 1)",
        "isLabelAutoCharLimit": true,
        "categoryType": "categorical",
        "isMaximumRangeEnabled": false,
        "maximumRange": 0,
        "isMinimumRangeEnabled": false,
        "minimumRange": 0,
        "isInvertRange": false,
        "isAutoDateFormat": true,
        "dateFormat": "DD:MM:YYYY hh:mm A",
        "customDateFormat": "DD:MM:YYYY hh:mm A",
        "numberFormatting": {
            "show": true,
            "valueType": "absolute",
            "decimalSeparator": ".",
            "thousandsSeparator": ",",
            "decimalPlaces": 1,
            "scaling": "auto",
            "prefix": "",
            "suffix": "",
            "scalingLabel": false,
            "thousandScalingLabel": "K",
            "millionScalingLabel": "M",
            "billionScalingLabel": "B",
            "trillionScalingLabel": "T"
        }
    }
}