...
import { ESmallMultiplesAxisType, ESmallMultiplesDisplayType, ESmallMultiplesHeaderPosition, ESmallMultiplesLayoutType, ESmallMultiplesViewType, ESmallMultiplesXAxisPosition, ESmallMultiplesYAxisPosition, Orientation } from "./SmallMultiples.enum";
import SmallMultiplesLayout from "./smallMultiplesLayout";
import { getSVGTextSize } from "../methods/methods";
...
export const GetGridLayoutRowsColumnsCount = (
    ...
    const fluidColumnsCount = Math.floor(config.containerWidth / minItemWidth) > 1 ? Math.floor(config.containerWidth / minItemWidth) : 2;
    const fluidRowsCount = Math.floor(config.containerHeight / minItemHeight) > 1 ? Math.floor(config.containerHeight / minItemHeight) : 2;
...
export const CreateSmallMultiplesPaginationPanel = (
    ...
    const paginationPanelInnerHtml = `
      <div class="pagination-panel-content">
        <span class="pagination-items-count">
          ${SMFirstItemIndexOnCurrentPage} to ${SMFirstItemIndexOnCurrentPage + SMItemsPerPage - 1} 
          of
          ${SMTotalItems}</span>
        ...
        <span class="pagination-page-count">
          Page ${SMCurrentPage} of ${SMTotalPages}
        </span>
        </span> 
        <button class="pagination-button goToNextPageButton">
            >
        </button>
    ...
    const setButtonDisabledProps = (SMPaginationPanel: D3Selection<HTMLDivElement>) => {
    ...
    SMPaginationPanel.html(paginationPanelInnerHtml);

    setButtonDisabledProps(SMPaginationPanel);
...
export const GetReactGridLayout = (config: ISmallMultiplesGridLayoutSettings, columns: number, totalRows: number): ILayoutItemProps[] => {
    ...
    for (let i = 0; i < Math.ceil(config.categories.length / columns); i++) {
        ...
        for (let j = i * columns; j < i * columns + columns; j++) {
            total += (config.gridDataItemsTotals[j] ?? 0);
        }
    ...
    if (config.layoutType === ESmallMultiplesLayoutType.Grid) {
        for (let i = 0; i < totalRows; i++) {
            for (let j = 0; j < columns; j++) {
                const obj = { i: config.categories[iterator], x: j, y: i, w: 1, h: 1, isDraggable: false, isResizable: true };
                layout.push(obj);
                iterator++;
            }
        }
        ...
        for (let i = 0; i < totalRows; i++) {
            ...
            for (let j = 0; j < columns; j++) {
                const obj = { i: config.categories[iterator], x: j, y: i, w: 1, h: height, isDraggable: false, isResizable: false };
                layout.push(obj);
                iterator++;
            }
        }
...
export const GetReactPaginationLayout = (config: ISmallMultiplesGridLayoutSettings, columns: number, totalRows: number, firstItem: number): ILayoutItemProps[] => {
    ...
    if (config.layoutType === ESmallMultiplesLayoutType.Grid) {
        for (let i = 0; i < totalRows; i++) {
            for (let j = 0; j < columns; j++) {
                const obj = { i: config.categories[iterator], x: j, y: i, w: 1, h: 1, isDraggable: true, isResizable: true };
                layout.push(obj);
                iterator++;
            }
        }
...
export const RenderSmallMultiplesUniformYAxis = (
    ...
    if (isUniformYScale) {
        ...
        hyperListMainContainer.on("scroll", () => {
            ...
            if (uniformRightYAxis) {
                uniformRightYAxis.style("transform", `translate(${config.containerWidth + config.smallMultiplesLayoutScrollbarWidth}px, ${-hyperListMainContainer.node().scrollTop}px)`);
            }
        });
...
export const RenderSmallMultiplesUniformXAxis = (
    ...
    if (isUniformXScale) {
        ...
        for (let i = 0; i < columns; i++) {
            ...
            if (isUniformXScale) {
                const { xAxisNodeHeight } = config.getUniformXAxisAndBrushNode(g.select(`#uniformXAxis-${i}`).node() as any, uniformBrushSVG.select(".brush").node() as any);
            }
