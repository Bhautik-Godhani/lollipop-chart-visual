...
import { CreateDate, GetWordsSplitByWidth, createMarkerDefs, createPatternsDefs, generatePattern, getSVGTextSize, hexToRGB, invertColorByBrightness, isConditionMatch1, isPercentageMatch, parseConditionalFormatting, powerBiNumberFormat, rgbaToHex } from "./methods/methods";
...
export class Visual extends Shadow {
	...
	smallMultiplesGridItemsList: { rowIndex: number, colIndex: number, content: ISmallMultiplesGridItemContent }[] = [];
				...
				if (!this.isHorizontalChart) {
					if (this.isAllNegativeValue) {
						if (((this.xAxisSettings.position === Position.Bottom && !this.yAxisSettings.isInvertRange)
							...
							return this.negativeLogScale((value === 0 ? -0.1 : value) * -1) + this.positiveLogScaleHeight;
						} else {
							return this.negativeLogScale((value === 0 ? -0.1 : value) * -1)
						}
					} else {
						if (((this.xAxisSettings.position === Position.Bottom && !this.yAxisSettings.isInvertRange)
							|| (this.xAxisSettings.position === Position.Top && this.yAxisSettings.isInvertRange))) {
							return value >= 0 ? this.positiveLogScale(value === 0 ? 0.1 : value) : this.negativeLogScale((value === 0 ? -0.1 : value) * -1) + this.positiveLogScaleHeight;
						} else {
							return value >= 0 ? this.positiveLogScale(value === 0 ? 0.1 : value) + this.negativeLogScaleHeight : this.negativeLogScale((value === 0 ? -0.1 : value) * -1)
						}
					}
				}
			}
	...
	setInitialChartData(
		...
		this.setVisualSettingsFromInitialChartData(categoricalData, categoricalCategoriesFields, categoricalCategoriesLastIndex, categoricalMeasureFields, categoricalSubCategoryField);
		this.setInitialChartDataExtended1(categoricalData, categoricalCategoriesFields, categoricalMeasureFields, categoricalTooltipFields, categoricalSortFields, categoricalRaceBarValues, categoricalSubCategoryField, categoricalCategoriesLastIndex, isOnlyCreateDataPairs);
		this.setInitialChartDataExtended2(categoricalData, categoricalMeasureFields, categoricalRaceBarValues, categoricalCategoriesFields, categoricalSortFields, categoricalTooltipFields, categoricalSmallMultiplesFields, categoricalCategoriesLastIndex);
		this.setInitialChartDataExtended3(categoricalData, width, height, isRenderBrush);

		return categoricalData;
	}

	setInitialChartDataExtended1(
		categoricalData: powerbi.DataViewCategorical,
		categoricalCategoriesFields: powerbi.DataViewCategoryColumn[],
		categoricalMeasureFields: powerbi.DataViewValueColumn[],
		categoricalTooltipFields: powerbi.DataViewValueColumn[],
		categoricalSortFields: powerbi.DataViewValueColumn[],
		categoricalRaceBarValues: powerbi.DataViewCategoryColumn[],
		categoricalSubCategoryField: any,
		categoricalCategoriesLastIndex: number,
		isOnlyCreateDataPairs: boolean
	) {
		if (this.isExpandAllApplied) {
		...
		this.setVisualMargeProperties(isOnlyCreateDataPairs);
		this.setVisualPropertiesValues(categoricalData);
	...
	setInitialChartDataExtended2(
		...
		categoricalSortFields: powerbi.DataViewValueColumn[],
		categoricalTooltipFields: powerbi.DataViewValueColumn[],
		categoricalSmallMultiplesDataFields: powerbi.DataViewCategoryColumn[],
		categoricalCategoriesLastIndex: number
	) {
	...
	setInitialChartDataExtended3(
		categoricalData: powerbi.DataViewCategorical,
		width: number, height: number, isRenderBrush: boolean
	) {
		const categoricalValues = categoricalData.values as any;
		const measures: any[] = categoricalValues.filter((d) => d.source.roles[EDataRolesName.Measure]);
		...
		this.displayBrushFromSetInitialChatData(categoricalData, width, height, isRenderBrush);
	}

	setVisualMargeProperties(isOnlyCreateDataPairs: boolean): void {
		if (!isOnlyCreateDataPairs) {
	...
	setVisualSettingsFromInitialChartData(
		categoricalData: powerbi.DataViewCategorical,
		categoricalCategoriesFields: powerbi.DataViewCategoryColumn[],
		categoricalCategoriesLastIndex: number,
		categoricalMeasureFields: powerbi.DataViewValueColumn[],
	...
	setVisualPropertiesValues(categoricalData: powerbi.DataViewCategorical): void {
	...
	// }
	// } else {
	setCategoricalDataPairs(
		categoricalData: powerbi.DataViewCategorical,
		categoricalSmallMultiplesFields: powerbi.DataViewCategoryColumn[],
		categoricalCategoriesLastIndex: number,
	): void {
	...
	// 	});
	// } else {
	setCategoricalDataFromPairs(categoricalData: powerbi.DataViewCategorical): void {
	...
	displayBrushFromSetInitialChatData(categoricalData: powerbi.DataViewCategorical, width: number, height: number, isRenderBrush: boolean): void {
	...
	setSmallMultiplesChartDataBySMCategory(width: number, height: number): void {
		...
		this.smallMultiplesCategories.forEach((d, i) => {
			const smallMultipleIndex = i;
			const isOthersSM = this.smallMultiplesCategories[smallMultipleIndex].toString().includes(this.othersLabel);
			const clonedCategoricalData: powerbi.DataViewCategorical = cloneDeep(this.originalCategoricalData);

			if (isOthersSM) {
				const SMRaking = this.rankingSettings.smallMultiples;
				let othersSMData: any[];

				if (SMRaking.enabled) {
					if (SMRaking.rankingType === ERankingType.TopN) {
						if (this.isHorizontalChart) {
							if (SMRaking.count <= this.smallMultiplesDataPairs.length) {
								othersSMData = this.smallMultiplesDataPairs.slice(SMRaking.count, this.smallMultiplesDataPairs.length);
							}
						} else {
							othersSMData = this.smallMultiplesDataPairs.slice(SMRaking.count, this.smallMultiplesDataPairs.length);
						}
					...
					if (SMRaking.rankingType === ERankingType.BottomN) {
						if (this.isHorizontalChart) {
							othersSMData = this.smallMultiplesDataPairs.slice(0, this.smallMultiplesDataPairs.length - SMRaking.count);
						} else {
							if (SMRaking.count <= this.smallMultiplesDataPairs.length) {
								othersSMData = this.smallMultiplesDataPairs.slice(0, this.smallMultiplesDataPairs.length - SMRaking.count);
							}
				...
				const smallMultiplesDataPair = this.smallMultiplesDataPairs.find((d) => d.category === this.smallMultiplesCategories[0]);
				const dataValuesIndexes = Object.keys(smallMultiplesDataPair).filter(key => key.includes("index-"));
				clonedCategoricalData.categories.forEach((d) => {
					d.values = dataValuesIndexes.map((valueIndex) => {
						const id = +valueIndex.split("-")[1];
				...
				clonedCategoricalData.values.forEach((d) => {
					const values: d3.Primitive[][] = [];

					if (d.source.roles[EDataRolesName.Measure]) {
						othersSMData.forEach(o => {
							const smallMultiplesDataPair = this.smallMultiplesDataPairs.find((d) => d.category === o.category);
							const dataValuesIndexes = Object.keys(smallMultiplesDataPair).filter(key => key.includes("index-"));
						...
						d.values = values[0].map((_, i) => (SMRaking.calcMethod === ERankingCalcMethod.Sum ? d3.sum(values, d => <number>d[i]) : d3.sum(values, d => <number>d[i]) / othersSMData.length));
				...
				this.smallMultiplesGridItemId = smallMultiplesDataPair.category;
			} else {
				const smallMultiplesDataPair = this.smallMultiplesDataPairs.find((d) => d.category === this.smallMultiplesCategories[smallMultipleIndex]);
				const dataValuesIndexes = Object.keys(smallMultiplesDataPair).filter(key => key.includes("index-"));
			...
			this.smallMultiplesGridItemsCategoricalData[d] = cloneDeep({
				...
				categoricalDataPairs: this.categoricalDataPairs
			});
	...
	setSmallMultiplesChartDataBySMCategoryExtended(data: any[]): void {
		...
		categoricalData.categories.forEach((d, i) => {
			if (d.source.roles[EDataRolesName.Category]) {
				if (i === this.categoricalCategoriesLastIndex) {
					d.values = data.map((pair) => pair.category);
				} else {
					d.values = data.map((pair) => pair[d.source.displayName]);
				}
			}

			if (d.source.roles[EDataRolesName.SmallMultiples]) {
				d.values = data.map((pair) => pair[`smallMultipleCategory-${d.source.index}`]);
			}
		});
		...
		categoricalData.values.forEach((d) => {
			if (Object.keys(d.source).includes("groupName")) {
				if (d.source.groupName !== null && d.source.groupName !== undefined && d.source.groupName !== "" && d.source.groupName !== this.blankText) {
					d.values = data.map((pair) => pair[`${Object.keys(d.source.roles)[0]}${d.source.index}${d.source.groupName}`]);
					d.highlights = data.map((pair) => pair[`${Object.keys(d.source.roles)[0]}${d.source.index}${d.source.groupName}Highlight`]);
				} else {
					d.values = data.map((pair) => pair[`${Object.keys(d.source.roles)[0]}${d.source.index}${this.blankText}`]);
					d.highlights = this.categoricalDataPairs.map((pair) => pair[`${Object.keys(d.source.roles)[0]}${d.source.index}${this.blankText}Highlight`]);
				}
			} else {
				d.values = data.map((pair) => pair[`${Object.keys(d.source.roles)[0]}${d.source.index}`]);
				d.highlights = data.map((pair) => pair[`${Object.keys(d.source.roles)[0]}${d.source.index}Highlight`]);
			}
		});

		this.SMCategoricalData = cloneDeep(categoricalData);

		this.setCategoricalDataFields(categoricalData);
		this.setChartData(categoricalData);

		this.SMChartData = cloneDeep(this.chartData);

		// const categories = this.isHorizontalChart ? this.chartData.map(d => d.category).reverse() : this.chartData.map(d => d.category);

		// this.uniformSMBrushScaleBandDomain = categories;
		// this.clonedUniformSMBrushScaleBandDomain = cloneDeep(categories);

		// const innerPadding = this.getXScaleInnerPadding();
		// this.brushScaleBand.domain(this.clonedUniformSMBrushScaleBandDomain);
	}
	...
	setConditionalFormattingColor(): void {
		const conditionalFormattingConditions: IConditionalFormattingProps[] = cloneDeep(this.conditionalFormattingConditions);
		...
		this.firstCFLine = conditionalFormattingConditions.filter(d => d.sourceName === this.subCategoryDisplayName).find(d => d.applyOnCategories.includes(ECFApplyOnCategories.Line));
		this.firstCFLabel = conditionalFormattingConditions.filter(d => d.sourceName === this.subCategoryDisplayName).find(d => d.applyOnCategories.includes(ECFApplyOnCategories.Labels));
	}

	setConditionalFormattingColorByValue(conditionalFormattingResult, d: ILollipopChartRow, c: IConditionalFormattingProps): void {
		if (this.isLollipopTypeCircle) {
	...
	setConditionalFormattingColorByPercentage(chartData: ILollipopChartRow[], d: ILollipopChartRow, c: IConditionalFormattingProps): void {
	...
	setPieColors(): void {
		...
		this.categoricalDataPairs.forEach(d => {
			this.subCategoriesName.forEach((c, i) => {
				...
				this.othersSubCategoryColorPair[`${d.category}-${c}`] = cloneDeep(this.subCategoryColorPairWithIndex[`${i}-${d.category}-${c}`]);
		...
		if (this.isSmallMultiplesEnabled && this.isCurrentSmallMultipleIsOthers && this.dataColorsSettings.isCustomizeSMOthersColor && this.rankingSettings.smallMultiples.enabled && this.rankingSettings.smallMultiples.showRemainingAsOthers) {
			const othersColor = this.dataColorsSettings.SMOthersColor;
			this.categoricalDataPairs.forEach((d) => {
				this.subCategoriesName.forEach((c) => {
					this.othersSubCategoryColorPair[`${d.category}-${c}`].marker1Color = othersColor;
					this.othersSubCategoryColorPair[`${d.category}-${c}`].marker2Color = othersColor;
					this.CFCategoryColorPair[`${d.category}-${c}`] = { isMarker1Color: false, isMarker2Color: false, isLineColor: false, isLabelColor: false };
				})
			})
		}
		...
		if (this.isHorizontalChart) {
			...
			if (this.height < newHeight || this.brushAndZoomAreaSettings.enabled) {
				...
				const config: IBrushConfig = {
					...
					isShowXAxis,
					isShowYAxis,
					isShowBrush: isShowBrush && isShowYAxis,
				};
			...
			if (this.width < newWidth || this.brushAndZoomAreaSettings.enabled) {
				...
				const config: IBrushConfig = {
					...
					isShowXAxis,
					isShowYAxis,
					isShowBrush: isShowBrush && isShowXAxis,
				};
	...
	verticalBrushed(config: IBrushConfig, selection, scaleHeight: number, brushLollipopChartMarker: IMarkerData): { isBrushRendered: boolean } {
		...
		if (this.isExpandAllApplied) {
			this.expandAllCategoriesName.forEach((d) => {
				if (this[`${d}ScaleDomain`]) { this[`${d}ScaleNewDomain`] = []; }
			});
		}

		if (config.isSMUniformYAxis) {
			const SMItemsList = this.smallMultiplesGridItemsList.filter(d => d.rowIndex === config.SMUniformYAxisId);

			if (config.isSMUniformYAxisIsLeft) {
				this.isLeftYAxis = true;
				this.yAxisSettings.position = Position.Left;
			} else {
				this.isLeftYAxis = false;
	...
	drawVerticalBrush(self: Visual, config: IBrushConfig): void {
		...
		const brush = d3
			...
			.extent([
				[0, 0],
				[this.brushWidth, height],
			])
			.on("brush", brushed);

		const expectedBar = this.brushAndZoomAreaSettings.enabled ? this.brushAndZoomAreaSettings.minLollipopCount : Math.ceil(scaleHeight / barDistance);
		const totalBar = totalBarsCount;
		const heightByExpectedBar = (expectedBar * height) / totalBar;
		...
		this.brushG
			...
			.attr("display", config.isShowBrush ? "block" : "none")
			.call(brush)
	...
	horizontalBrushed = (config: IBrushConfig, categoricalData: powerbi.DataViewCategorical, selection, scaleWidth: number, brushLollipopChartMarker: IMarkerData): { brushG: SVGElement, isBrushRendered: boolean } => {
		...
		if (config.isSMUniformXAxis) {
			const SMItemsList = this.smallMultiplesGridItemsList.filter(d => d.colIndex === config.SMUniformXAxisId);

			if (config.isSMUniformXAxisIsBottom) {
				this.isBottomXAxis = true; this.xAxisSettings.position = Position.Bottom;
			} else {
				this.isBottomXAxis = false; this.xAxisSettings.position = Position.Top;
			}
	...
	drawHorizontalBrush(self: Visual, config: IBrushConfig): void {
		...
		const brush = d3
			...
			.extent([
				[0, 0],
				[width, this.brushHeight - (this.brushAndZoomAreaSettings.enabled ? 2 : 0)],
			])
			.on("brush", brushed);
		...
		widthByExpectedBar = widthByExpectedBar > width ? width : widthByExpectedBar;
	...
	setDataLabelsFormatting(labelSelection: D3Selection<SVGElement>, textSelection: any, isData2Label: boolean = false, labelPlacement: DataLabelsPlacement, isBestFitPlacement: boolean): void {
		...
		this.setDataLabelsFormattingExtended(labelSelection, textSelection, labelPlacement, fontSize, isAutoBGColor, isData2Label);
	}

	setDataLabelsFormattingExtended(labelSelection: D3Selection<SVGElement>, textSelection: D3Selection<SVGElement>, labelPlacement: DataLabelsPlacement, fontSize: number, isAutoBGColor: boolean, isData2Label: boolean): void {
		const dataLabelsSettings = isData2Label ? this.data2LabelsSettings : this.data1LabelsSettings;
	...
	drawTooltip(): void {
		...
		const getTooltipData = (value: ILollipopChartRow, isCircle1: boolean, isMultiMeasure: boolean): VisualTooltipDataItem[] => {
			const categoryColorPair = this.isSmallMultiplesEnabled && value.isOthersSmallMultiples ? this.othersCategoryColorPair : this.categoryColorPair;
			...
			if (isMultiMeasure) {
				tooltipData.push({
					...
					color: (isPosNegColorScheme2 ? posNegColor2 : categoryColorPair[value.category].marker2Color),
				})
			}
	...
	setXAxisDomain(categoricalData: powerbi.DataViewCategorical, chartData: ILollipopChartRow[]): void {
		const values = categoricalData.categories[this.categoricalCategoriesLastIndex].values;
		...
		if (this.isXIsContinuousAxis || this.isYIsContinuousAxis) {
			if (((this.isXIsDateTimeAxis || this.isDateCategoryNames) && this.isXIsContinuousAxis) || (this.isYIsDateTimeAxis && this.isYIsContinuousAxis)) {
				...
				const values = chartData.map(d => parseFloat(d.category));
			...
			this.xScale = d3.scaleBand();
			this.xScale.domain(chartData.map((d) => d.category));
		}
	}

	setYAxisDomain(chartData: ILollipopChartRow[],): void {
		...
		const isLinearScale: boolean = typeof chartData.map((d) => d.value1)[0] === "number" && !this.isLogarithmScale;
		...
		if (isLinearScale) {
			...
			this.yScale.domain(chartData.map((d) => d.value1));
		}
	}
	...
	setXYAxisRange(xScaleWidth: number, yScaleHeight: number): void {
		...
		if (this.isHorizontalChart) {
			...
			if (this.isYIsContinuousAxis) {
				this.yScale.range(this.xAxisSettings.position === Position.Bottom ? [yScaleHeight - this.yAxisStartMargin - this.markerMaxSize, this.markerMaxSize] : [this.yAxisStartMargin + this.markerMaxSize, yScaleHeight - this.markerMaxSize]);
			} else {
	...
	drawXGridLines(): void {
		...
		if (this.isHorizontalChart) {
			if (this.isCutAndClipAxisEnabled) {
				...
				if (this.isShowPositiveNegativeLogScale || this.isAllNegativeValue) {
					xScaleTicks = [
	...
	setPath1Formatting(circleSelection: any): void {
		...
		circleSelection
			...
			.each((d: ILollipopChartRow, i: number, nodes) => {
				...
				const isCFColor = this.CFCategoryColorPair[d.category] && this.CFCategoryColorPair[d.category].isMarker1Color;
				const isPosNegColorScheme = this.dataColorsSettings.fillType === ColorPaletteType.PositiveNegative && !isCFColor;
				const posNegColor = d.value1 >= 0 ? this.dataColorsSettings.positiveColor : this.dataColorsSettings.negativeColor;
				const categoryColorPair = this.isSmallMultiplesEnabled && d.isOthersSmallMultiples && !isCFColor ? this.othersCategoryColorPair : this.categoryColorPair;
				...
				if (((d.category === this.othersBarText)) && this.dataColorsSettings.isCustomizeCategoryOthersColor && !isCFColor) {
	...
	getPieFill = (d: IChartSubCategory, parent: ILollipopChartRow, isPie2: boolean) => {
		let color;
		let isHasPattern: boolean;
	...
	getPieChartSeriesDataByCategory(category: string, isPie2: boolean = false) {
		...
		const data = this.chartData[id].subCategories.map((data) => {
			const { color, pieFill, isHasPattern, pattern } = this.getPieFill(data, this.chartData[id], isPie2);
			let color1 = pieFill;
			let color2 = pieFill;
	...
	configurePieChart(d: ILollipopChartRow, ele: D3Selection<SVGElement>, i: number, isPie2: boolean): void {
		...
		const smallMultiplesGridItemId = this.smallMultiplesGridItemId;
		ele.selectAll(".pie-slice").each(function (_, i) {
			...
			d3.select(this).datum((datum: any) => {
				...
				return {
					...
					smallMultipleCategory: smallMultiplesGridItemId
				}
			})
		})
